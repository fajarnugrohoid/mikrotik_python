...
class Core:
	...
	def openMsgCloseSocket(self):
		easygui.msgbox("Aya nu mantau!!!", title="Alert")
	...
	def send_message(self, channel_id, message):
		self.slack_client.api_call("chat.postMessage",channel=channel_id,text=message,username='pythonbot',icon_emoji=':robot_face:')
	...
	def sendMsgToSlack(self):
		...
		if channels:
			...
			for channel in channels:
				...
				detailed_info = self.channel_info(channel['id'])
				if detailed_info:
					print('Latest text from ' + channel['name'] + ":")
					print(detailed_info['latest']['text'])
				if channel['name'] == 'general':
					self.send_message(channel['id'], "dipantau @everyone ip : 218.45.174.214")
			print('-----')
		else:
	...
	def readWord(self):
		...
		threads = []
		#target=self.openWarning()
		if self.DEBUG:
			if ret=="=dst-address=218.45.174.214":
				print "run alert!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"
				self.sendMsgToSlack()
				self.openMsgCloseSocket()
				#close_until_seconds=3
				#t = Thread(target=self.openWarning)
				#threads.append(t)
				#t.start()
			print ">>> " + ret
		return ret
				...
				#close_until_seconds=3
				#text="111"
				#t = Thread(target=self.worker,args=(title,1))
				#t.start()
				...
				#easygui.msgbox("Aya nu mantau!!!", title="Alert")
					...
					#self.sk.close()
					#sys.exit(0)
			#if ret=="=src-address=218.45.174.214":
			#if ret=="=src-address=192.168.101.160":
			#if ret=="=dst-address=192.168.101.254":
			#if ret=="=dst-address=202.134.0.155":
			#if ret=="=dst-address=203.180.216.128":
			#if ret=="=dst-address=202.214.115.96":
	...
	def readStr(self, length):
		...
		while len(ret) < length:
			print "length:" + str(length) + " - len-ret:" + str(len(ret))
			print >>sys.stderr, '\nwaiting for the next event'
			self.sk.setblocking(0)
			print "after setblocking"
			print self.sk
			if not self.sk:
				continue
			try:
				readable, writable, exceptional = something.select([self.sk], [], [], 0)
			except something.error, e:
				#code, message = e
				print "e:" + e
			print "after something.select"			
...
def test():
	tik = Core("192.168.101.254", DEBUG=True)
	tik.login("user1", "user")
	tik.run_interpreter()
